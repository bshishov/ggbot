...
__all__ = ["SteamApi", "STEAM_ID_32_TO_64_OFFSET", "to_steam_id_64", "to_steam_id_32"]
...
class SteamApi:
    ...
    async def get_app_list(self):
        url = (
            f"http://api.steampowered.com/ISteamApps/GetAppList/v2"
            f"?key={self.key}"
    ...
    async def get_resolve_vanity_url(self, vanity_url: StrOrInt):
        url = (
            f"http://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001"
            f"?key={self.key}"
    ...
    async def get_dota_match_details(self, match_id: StrOrInt, app_id: StrOrInt = 570):
        url = (
            f"http://api.steampowered.com/IDOTA2Match_{app_id}/GetMatchDetails/v1"
            f"?key={self.key}"
            f"&match_id={match_id}"
            f"&language={self.language}"
            f"&format=json"
