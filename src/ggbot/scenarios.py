    ...
    repeat_until_timer_expires(
        ...
        selector(
            sequence(
                ...
                set_var('gathered', '{% if gathered is defined %}{{ gathered + [message.author.name] }}{% else %}{{ [message.author.name] }}{% endif %}'),
                edit_last_answer('(это тест) Кто пойдет в доту? (20 секунд жду) `{{ gathered }}`'),
            ),
...
DIALOG_TEST_SCENARIO = sequence(
    ...
    retry_until_success(2, sequence(
        sequence(
            wait_for_message_from_user_with_intents(['yes', 'no'], seconds=5),
            selector(
                sequence(
                    message_intent_is('yes'),
                    reply_to_message('Ок')
                ),
                sequence(
                    message_intent_is('no'),
                    reply_to_message('Нет так нет')
                ),
                always_fail(
                    random_one_of(
