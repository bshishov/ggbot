...
__all__ = ["IgdbClient", "get_access_token"]
...
class IgdbClient(BotComponent):
    ...
    async def query(self, endpoint: str, query: str):
        ...
        async with self.get_session() as session:
            response = await session.post(
                f"https://api.igdb.com/v4/{endpoint}", data=query
            )
    ...
    async def action_game_embed_results(
        ...
        for game in results[:limit]:
            ...
            pprint.pprint(game)
            embed = discord.Embed(title=game["name"], type="rich", url=game["url"])
            if "cover" in game:
                embed.set_thumbnail(url="https:" + game["cover"]["url"])

            if "aggregated_rating" in game:
                embed.add_field(
                    name="Рейтинг", value=str(int(game["aggregated_rating"]))
                )
